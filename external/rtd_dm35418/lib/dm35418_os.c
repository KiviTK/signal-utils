/**
	...
	$Id: dm35418_os.c 119521 2019-04-26 10:32:22Z prucz $
*/
...
int DM35418_Dma_Initialize(struct DM35418_Board_Descriptor *handle,
				...
				uint32_t buffer_size)
{
	 ...
	 * Disable interrupts
	 */
	result = DM35418_Dma_Configure_Interrupts(handle,
						func_block,
						channel,
						0,
						0);
	...
	result = DM35418_Dma_Clear(handle,
					func_block,
					channel);
	...
	result = DM35418_Dma_Clear_Interrupt(handle,
						func_block,
						channel,
						1,
						1,
	...
	for (buff = 0; buff < num_buffers; buff++) {
		 ...
		 * Allocate buffer
		 */
		ioctl_request.dma.pci.region = DM35418_PCI_REGION_FB;
		ioctl_request.dma.pci.size = DM35418_PCI_REGION_ACCESS_32;
		ioctl_request.dma.function = DM35418_DMA_INITIALIZE;
		...
		ioctl_request.dma.pci.offset = func_block->dma_channel[channel].buffer_start_offset[buff] +
						DM35418_OFFSET_DMA_BUFFER_ADDRESS;
		ioctl_request.dma.buffer = buff;
        	result = DM35418_Dma(handle, &ioctl_request);
		check_result(result, "Error initializing DMA via ioctl");

		result = DM35418_Dma_Buffer_Set_Size(handle,
							func_block,
							channel,
							buff,
							(buffer_size & DM35418_BIT_MASK_DMA_BUFFER_SIZE));
		 ...
		 * Clear the status
		 */
		result = DM35418_Dma_Reset_Buffer(handle,
							func_block,
							channel,
							buff);
		 ...
		 */
		result = DM35418_Dma_Buffer_Setup(handle,
							func_block,
							channel,
							buff,
							DM35418_DMA_BUFFER_CTRL_CLEAR);
				...
				void *local_buffer_ptr)
{
	...
	ioctl_request.dma.function = DM35418_DMA_READ;
	ioctl_request.dma.channel = channel;
	ioctl_request.dma.fb_num = func_block->fb_num;
	ioctl_request.dma.buffer_size = buffer_size;
	...
	return DM35418_Dma(handle, &ioctl_request);
				...
				void *local_buffer_ptr)
{
	...
	ioctl_request.dma.function = DM35418_DMA_WRITE;
	ioctl_request.dma.channel = channel;
	ioctl_request.dma.fb_num = func_block->fb_num;
	ioctl_request.dma.buffer_size = buffer_size;
	ioctl_request.dma.buffer_ptr = local_buffer_ptr;
...
int DM35418_General_RemoveISR(struct DM35418_Board_Descriptor *handle)
{
	...
	ioctl(handle->file_descriptor, DM35418_IOCTL_WAKEUP);
...
void *DM35418_General_WaitForInterrupt(void *ptr)
{
	...
	handle = (struct DM35418_Board_Descriptor *) ptr;
	union dm35418_ioctl_argument ioctl_arg;

	while (1) {

		/*
		 * Set up the set of file descriptors that will be watched for input
		 * activity.  Only the DM35418 device file descriptor is of interest.
		 */
		 ...
		 * activity.  Only the DM35418 file descriptor is of interest.
		 */
		...
		status = ioctl(handle->file_descriptor, DM35418_IOCTL_INTERRUPT_GET,
			  &ioctl_arg);
...
DM35418_General_InstallISR(struct DM35418_Board_Descriptor *handle, void (*isr_fnct))
{
	...
	if (pthread_create
		(&(handle->pid), NULL, DM35418_General_WaitForInterrupt,
		 handle) != 0) {
		errno = EFAULT;
		return -1;
	}
...
int 
DM35418_General_SetISRPriority(struct DM35418_Board_Descriptor *handle,
				   int priority)
{
