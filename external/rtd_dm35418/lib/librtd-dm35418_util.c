/**
	...
	@brief
		DM35418 Util library source code

	$Id: librtd-dm35418_util.c 107475 2017-03-28 15:17:54Z rgroner $
*/
...
#define DM35418_ONE_SEC_IN_MICRO	1000000
#define DM35418_MICRO_TO_NANO(x)	((x) * 1000)
#define DM35418_SEC_TO_MICRO(x)	((x) * 1000000)


long DM35418_Get_Time_Diff(struct timeval last, struct timeval first) {
	long firstTime;
	long lastTime;

	lastTime = DM35418_SEC_TO_MICRO(last.tv_sec) + last.tv_usec;
	firstTime = DM35418_SEC_TO_MICRO(first.tv_sec) + first.tv_usec;

	return (lastTime - firstTime);
}


void DM35418_Micro_Sleep(unsigned long microsecs) {

	struct timespec sleep_time, temp_time;
	long seconds = microsecs / DM35418_ONE_SEC_IN_MICRO;

	microsecs = microsecs % DM35418_ONE_SEC_IN_MICRO;

	sleep_time.tv_sec = seconds;
	sleep_time.tv_nsec = DM35418_MICRO_TO_NANO(microsecs);
...
uint32_t DM35418_Get_Maskable(uint16_t data, uint16_t mask)
{
...
int DM35418_Generate_Signal_Data(enum DM35418_Waveforms waveform,
					int32_t *data,
					uint32_t data_count,
					int32_t max,
					int32_t minimum,
